...
class FFmpegReader():
    ...
    def __init__(self, filename, inputdict=None, outputdict=outputdict, verbosity=0):
        ...
        israw = 0
        sv=cv2.VideoCapture(filename)
        numframet=int(sv.get(cv2.CAP_PROP_FRAME_COUNT))
        sv.release()
        if not inputdict:
            inputdict = {}
    ...
    def _read_frame_data(self):
        # Init and check
        framesize = self.outputdepth * self.outputwidth * self.outputheight*2
        assert self._proc is not None
        tttt=self.outputdepth * self.outputwidth * self.outputheight
        try:
            # Read framesize bytes

            arr = np.fromstring(self._proc.stdout.read(framesize), dtype=np.uint16)
            assert len(arr) == tttt
